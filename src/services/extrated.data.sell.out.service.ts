...
export class ExtratedDataSelloutService {
  ...
  constructor(dataSource: DataSource) {
    this.extractedDataSelloutRepository = new ExtractedDataSelloutRepository(
      dataSource
    );
    this.userRepository = new UserRepository(dataSource);
    this.consolidatedDataStoresService = new ConsolidatedDataStoresService(
      dataSource
    );
    this.selloutConfigurationRepository = new SelloutConfigurationRepository(
      dataSource
    );
    ...
    this.createExtractedDataSellout =
      this.createExtractedDataSellout.bind(this);
    this.updateExtractedDataSellout =
      this.updateExtractedDataSellout.bind(this);
    this.deleteExtractedDataSellout =
      this.deleteExtractedDataSellout.bind(this);
    this.extractionLogsSelloutRepository = new ExtractionLogsSelloutRepository(
      dataSource
    );
    this.getFilteredExtractedDataSellout =
      this.getFilteredExtractedDataSellout.bind(this);
  }
  ...
  async createExtractedDataSellout(
    ...
    | ExtractedDataSelloutResponse
    | string
    | { distributor: string; storeName: string; status: boolean }
  > {
    try {
      const start = new Date();
      let isRepet = false;
      const entityExtractedData = await this.prepareExtractedEntity(
        dto,
        userConsenso
      );
      ...
      const template = await this.matriculationTemplateRepository.findById(
        dto.matriculationId!
      );
      if (!template) {
        throw new Error(
          `No se encontró la plantilla de matriculación con ID ${dto.matriculationId}`
        );
      }
      if (logMatriculationData && logMatriculationData.length > 0) {
        for (const log of logMatriculationData) {
          const existing =
            await this.matriculationLogsRepository.findByMatriculationIdAndCalculateDate(
              template.id!,
              dto.calculateDate!,
              log.distributor ?? "",
              log.storeName ?? ""
      ...
      if (isRepet) {
        throw new Error(
          `El archivo ya ha sido cargado previamente para la fecha ${dto.calculateDate}`
        );
      }
      // Paso 4: Procesar los datos consolidados
      const selloutConfig = await this.selloutConfigurationRepository.findById(
        dataConsolidado.selloutConfigurationId
      );
      const result =
        await this.consolidatedDataStoresService.processConsolidatedDataStores(
          records,
          template.id,
          dto.calculateDate!
        );
      ...
      throw new Error(
        `Error al procesar los datos extraídos: ${(error as Error).message}`
      );
    }
  }

  private async deleteLogAndAllConsolidatedDataStores(
    templateId: number,
    calculateDate: string
  ): Promise<void> {
    const existingLog =
      await this.matriculationLogsRepository.findByCalculateDateOne(
        calculateDate,
        templateId
      );
    if (existingLog) {
  ...
  private parseDataContent(dto: CreateExtractedDataSelloutDto): any {
    ...
    const blockName = data.consolidated_data_stores
      ? "consolidated_data_stores"
      : "";
    if (
      !blockName ||
      !Array.isArray(data[blockName]) ||
      data[blockName].length === 0
    ) {
      throw new Error(
        `El bloque '${blockName}' no contiene registros o no es un array`
      );
    }
    return { ...data, dataBlockName: blockName };
  }

  private parseDataLogs(
    dto: CreateExtractedDataSelloutDto
  ): MatriculationLog[] {
  ...
  private async processExtractedData(
    ...
    return plainToClass(
      ExtractedDataSelloutDto,
      { ...extractedData, productCount: productCount },
      {
  ...
  async updateExtractedDataSellout(
    id: number,
    selloutConfiguration: UpdateExtractedDataSelloutDto
  ): Promise<ExtractedDataSellout> {
    const existingExtractedDataSellout =
      await this.extractedDataSelloutRepository.findById(id);

    if (!existingExtractedDataSellout) {
      throw new Error(`Extracted data sellout con ID ${id} no encontrado`);
    }

    existingExtractedDataSellout.selloutConfiguration = {
      id: selloutConfiguration.selloutConfigurationId,
    } as SelloutConfiguration;
    existingExtractedDataSellout.extractionLogsSellout = {
      id: selloutConfiguration.extractionLogId,
    } as ExtractionLogsSellout;
    ...
    existingExtractedDataSellout.processedDate =
      selloutConfiguration.processedDate;
    existingExtractedDataSellout.processedBy = {
      id: selloutConfiguration.processedBy,
    } as User;
    existingExtractedDataSellout.processingDetails =
      selloutConfiguration.processingDetails;
    existingExtractedDataSellout.dataName = selloutConfiguration.dataName;
    existingExtractedDataSellout.calculateDate = parseDateFromISO(
      selloutConfiguration.calculateDate!
    );
    existingExtractedDataSellout.createdBy = {
      id: selloutConfiguration.createdBy,
    } as User;

    return this.extractedDataSelloutRepository.update(
      id,
      existingExtractedDataSellout
    );
  }
  ...
  async getFilteredExtractedDataSellout(
    ...
    const { items, total } =
      await this.extractedDataSelloutRepository.findPaginated(page, limit);
    return {
