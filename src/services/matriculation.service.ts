...
export class MatriculationService {
  ...
  constructor(dataSource: DataSource) {
    ...
    this.matriculationTemplateRepository = new MatriculationTemplatesRepository(
      dataSource
    );
    ...
    this.consolidatedDataStoresRepository =
      new ConsolidatedDataStoresRepository(dataSource);
  }
  ...
  async createMatriculationTemplate(
    ...
    const existingTemplate =
      await this.matriculationTemplateRepository.findByDistributorAndStoreName(
        matriculationTemplate.distributor!,
        matriculationTemplate.storeName!,
        parseLocalDate(matriculationTemplate.calculateMonth!)
      );
    if (existingTemplate) {
      throw new Error("Nombre de plantilla ya existe para este mes de cálculo");
    }

    const matriculationTemplateEntity = plainToInstance(
      MatriculationTemplate,
      matriculationTemplate,
      {}
    );
    const matriculationTemplateSaved =
      await this.matriculationTemplateRepository.create(
        matriculationTemplateEntity
      );
    return plainToInstance(
      MatriculationTemplateResponseDto,
      matriculationTemplateSaved,
      {
  ...
  async createMatriculationTemplatesBatch(
    configs: CreateMatriculationTemplateDto[]
  ): Promise<void> {
    const uniqueMap = new Map<string, CreateMatriculationTemplateDto>();

    for (const config of configs) {
      const distributor =
        config.distributor?.toString().replace(/ /g, "") ?? "";
      const storeName = config.storeName?.toString().replace(/ /g, "") ?? "";
      const calculateMonth =
        config.calculateMonth?.toString().replace(/ /g, "") ?? "";
    ...
    for (const chunk of chunks) {
      ...
      const existingStores =
        await this.matriculationTemplateRepository.findByDistributorAndStoreNameArray(
          distributorList,
          storeNameList,
          parseLocalDate(chunk[0].calculateMonth!)
        );

      const existingMap = new Map(
        existingStores.map((p: MatriculationTemplate) => [
          p.distributor + p.storeName,
          p,
        ])
      );
      ...
      for (const config of chunk) {
        const existing = existingMap.get(
          config.distributor! + config.storeName!
        );
        if (existing) {
  ...
  async createMatriculationTemplateBeforeMonth(
    calculateMonth: string,
    copyMonth: string
  ): Promise<{ message: string }> {
    const matriculationTemplates =
      await this.matriculationTemplateRepository.findByCalculateMonth(
        parseLocalDate(copyMonth)
      );
    ...
    return {
      message: `Se han creado ${createdTemplates} plantillas y se han encontrado ${existingTemplates} plantillas existentes para el mes ${calculateMonth}`,
    };
  }
  ...
  async updateMatriculationTemplate(
    ...
    matriculationTemplateEntity.distributor =
      matriculationTemplate.distributor!;
    matriculationTemplateEntity.calculateMonth =
      matriculationTemplate.calculateMonth
        ? parseLocalDate(matriculationTemplate.calculateMonth)
        : undefined;
    ...
    const matriculationTemplateUpdated =
      await this.matriculationTemplateRepository.update(
        id,
        matriculationTemplateEntity
      );
    return plainToInstance(
      MatriculationTemplateResponseDto,
      matriculationTemplateUpdated,
      {
        excludeExtraneousValues: true,
      }
    );
  ...
  async deleteMatriculationTemplateAll(
    deleteData: DeleteMatriculationTemplateDto
    ...
    for (const id of idsToDelete) {
      const existingTemplateLog =
        await this.matriculationLogsRepository.findByMatriculationIdAndDate(
          id,
          deleteData.fechaCalculo
        );
      if (existingTemplateLog) {
        noDeletedIds.push(
          `ID ${id}: No se pudo eliminar por tener logs de matrícula asociados (Distribuidor: ${existingTemplateLog.distributor}, Tienda: ${existingTemplateLog.storeName}). Elimine primero los logs.`
        );
      }
    }
    if (noDeletedIds.length > 0) {
      return noDeletedIds;
    }
    try {
      const successfullyDeletedCount =
        await this.matriculationTemplateRepository.deleteMany(idsToDelete);
      if (successfullyDeletedCount === idsToDelete.length) {
        return "Todas las plantillas fueron eliminadas correctamente.";
      } else {
        return `${successfullyDeletedCount} de ${idsToDelete.length} plantillas eliminadas correctamente.`;
      }
    } catch (error) {
      return [
        "Ocurrió un error inesperado al intentar eliminar las plantillas.",
      ];
    }
  }
  ...
  async getMatriculationTemplatesWithFilters(
    ...
    const filteredTemplates = templatesWithLogs.map((template) => {
      ...
      const rowCountTotal = filteredLogs.reduce(
        (acc, log) => acc + (log.rowsCount ?? 0),
        0
      );
      const productCountTotal = filteredLogs.reduce(
        (acc, log) => acc + (log.productCount ?? 0),
        0
      );
  ...
  async getMatriculationTemplates(
    ...
    const { items, total } =
      await this.matriculationTemplateRepository.findByFilters(
        page,
        limit,
        search,
        calculateMonth?.toString().slice(0, 7)
      );
    return {
      items: items.map((item) =>
        plainToInstance(MatriculationTemplateResponseDto, item, {
          excludeExtraneousValues: true,
        })
      ),
      total,
  ...
  async createMatriculationLog(
    ...
    const existingTemplate =
      await this.matriculationTemplateRepository.findById(
        matriculationLog.matriculationId!
      );
    if (!existingTemplate) {
      throw new Error("Matriculación de plantilla no encontrada");
    }
    const matriculationLogEntity = plainToInstance(
      MatriculationLog,
      matriculationLog,
      {}
    );
    ...
    const matriculationLogSaved = await this.matriculationLogsRepository.create(
      matriculationLogEntity
    );
  ...
  async updateMatriculationLog(
    ...
    const matriculationLogSaved = await this.matriculationLogsRepository.update(
      id,
      matriculationLogEntity
    );
    return plainToInstance(MatriculationLogResponseDto, matriculationLogSaved, {
  ...
  async deleteMatriculationLog(id: number): Promise<void> {
    ...
    await this.consolidatedDataStoresRepository.deleteAllByMatriculationId(
      id,
      existingLog.calculateDate
    );
    await this.matriculationLogsRepository.delete(id);
  }

  async getMatriculationLogs(
    calculateDate: string
  ): Promise<MatriculationLogResponseDto[]> {
    const matriculationLogs =
      await this.matriculationLogsRepository.findByCalculateDate(calculateDate);
  ...
  async hasBeenUploaded(
    ...
    const closingConfig =
      await this.closingConfigurationRepository.findByMonthYear(
        Number(year),
        Number(month)
      );
    if (!closingConfig) {
    ...
    const dateClosingISO = new Date(closingConfig.closingDate)
      .toISOString()
      .split("T")[0];
    ...
    const startDateISO = new Date(closingConfig.startDate!)
      .toISOString()
      .split("T")[0];
    const startDateNumber = startDateISO.split("-")[1];

    const closingDate = normalizeToStartOfDay(
      new Date(
        Number(dateClosingYear),
        Number(dateClosingMonth) - 1,
        Number(dateClosingDay)
      )
    );
    const closingMonth = createUTCDateOnly(
      closingDate.getFullYear(),
      Number(startDateNumber) - 1,
      1
    );
    ...
    if (template && !template?.status) {
      return {
        message:
          "La plantilla no está habilitada para esta fecha de cálculo, por favor habilítela.",
        status: false,
      };
    }
    ...
    if (isInRange) {
      if (!log) {
        ...
        return {
          message:
            "Ya se han cargado datos previamente con esta plantilla. Si continúas, la información actual será reemplazada.",
          status: true,
        };
      }
    }
    ...
    return {
      message:
        "La acción no puede completarse porque la fecha actual está fuera del rango permitido para el cierre.",
      status: false,
    };
  }
}
